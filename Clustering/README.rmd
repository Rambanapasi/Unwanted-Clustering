...
which stocks have moved, is the upward move justified and those that didnt reflect it what could be the catalysts that moves them. 
...
p <- p +
  ...
  expand_limits(x = c(-1, 35)) +  # Adjust x-axis limits for better spacing
  geom_text(
    data = filter(hcdata$labels, label %in% labelling),  # Filter labels for selected stocks
